require "application_system_test_case"

class ContactsTest < ApplicationSystemTestCase
  setup do
    @contact = contacts(:one)
  end

  test "visiting the index" do
    visit contacts_url
    assert_selector "h1", text: "Contacts"
  end

  test "creating a Contact" do
    visit contacts_url
    click_on "New Contact"

    fill_in "Avatar", with: @contact.avatar
    fill_in "Email", with: @contact.email
    fill_in "Facebook", with: @contact.facebook
    fill_in "Fname", with: @contact.fname
    fill_in "Highlights", with: @contact.highlights
    fill_in "Linked In", with: @contact.linked_in
    fill_in "Lname", with: @contact.lname
    fill_in "Notes", with: @contact.notes
    fill_in "Other Social", with: @contact.other_social
    fill_in "Phone", with: @contact.phone
    fill_in "Title", with: @contact.title
    fill_in "Twitter", with: @contact.twitter
    fill_in "User", with: @contact.user_id
    fill_in "Website", with: @contact.website
    click_on "Create Contact"

    assert_text "Contact was successfully created"
    click_on "Back"
  end

  test "updating a Contact" do
    visit contacts_url
    click_on "Edit", match: :first

    fill_in "Avatar", with: @contact.avatar
    fill_in "Email", with: @contact.email
    fill_in "Facebook", with: @contact.facebook
    fill_in "Fname", with: @contact.fname
    fill_in "Highlights", with: @contact.highlights
    fill_in "Linked In", with: @contact.linked_in
    fill_in "Lname", with: @contact.lname
    fill_in "Notes", with: @contact.notes
    fill_in "Other Social", with: @contact.other_social
    fill_in "Phone", with: @contact.phone
    fill_in "Title", with: @contact.title
    fill_in "Twitter", with: @contact.twitter
    fill_in "User", with: @contact.user_id
    fill_in "Website", with: @contact.website
    click_on "Update Contact"

    assert_text "Contact was successfully updated"
    click_on "Back"
  end

  test "destroying a Contact" do
    visit contacts_url
    page.accept_confirm do
      click_on "Destroy", match: :first
    end

    assert_text "Contact was successfully destroyed"
  end
end
